...
def preservation_tokens(value: str) -> list[str]:
    normalized = unicodedata.normalize("NFKC", value).casefold()
    return re.findall(r"[^\W_]+", normalized, flags=re.UNICODE)


def contains_preserved_phrase(text: str, phrase: str) -> bool:
    text_tokens = preservation_tokens(text)
    phrase_tokens = preservation_tokens(phrase)
...
def phrases_preserve_tokens_and_order(left: str, right: str) -> bool:
    left_tokens = preservation_tokens(left)
    return bool(left_tokens and left_tokens == preservation_tokens(right))
...
def contains_component(text: str, component: str) -> bool:
    ...
    if not component:
        return True
    if re.fullmatch(r"[A-Za-z0-9]+", component):
...
def contains_sku_attribute(text: str, attribute: str) -> bool:
    if contains_component(text, attribute):
        return True
    compact_text = re.sub(r"[\s,./-]+", "", normalize(text).casefold())
    compact_attribute = re.sub(r"[\s,./-]+", "", normalize(attribute).casefold())
    return bool(compact_attribute and compact_attribute in compact_text)
...
def validate(
    title_value: str,
    highlights_value: str,
    marketplace: str,
    brand: str = "",
    core_product_phrase: str = "",
    differentiator: str = "",
    sku_attributes: list[str] | None = None,
    sku_attribute_placement: str = "",
    sku_fit_base_title: str = "",
    required_core_keyword: str = "",
) -> dict:
    title = normalize(title_value)
    highlights = normalize(highlights_value)
    errors: list[str] = []
    warnings: list[str] = []
    sku_attributes = [normalize(item) for item in (sku_attributes or []) if normalize(item)]
    required_core_keyword = normalize(required_core_keyword)
    ...
    elif len(title) >= TITLE_HEADROOM_WARNING_AT:
        warnings.append(
            f"Title has {len(title)} characters and only {TITLE_LIMIT - len(title)} "
            "characters of headroom; prefer a shorter equally strong candidate."
        )
    ...
    highlight_phrases = split_highlight_phrases(highlights)
    informative_highlight_phrases = [
        phrase for phrase in highlight_phrases
        if len(content_words(phrase)) >= 2 or bool(re.search(r"\d", phrase))
    ]
    if len(highlight_phrases) == 1 and len(highlights) < 40:
        warnings.append("Item Highlights is unusually sparse; review the title-pair coverage audit.")
    if highlight_phrases and not informative_highlight_phrases:
        warnings.append("Item Highlights contains only isolated labels; add supported decision detail when available.")

    unit_spacing_scan = re.sub(
        r"\d+(?:[.,]\d+)?(?:\s*[x×]\s*\d+(?:[.,]\d+)?)+"
        r"(?:kg|g|cm|mm|ml|oz|in)\b",
        "",
        combined_casefold,
    )
    ...
    title_components = {
        ...
        "core_product_phrase": normalize(core_product_phrase),
        "differentiator": normalize(differentiator),
    }
    component_context = any(title_components.values()) or bool(sku_attributes) or bool(
        sku_attribute_placement
    )
    if component_context:
        for label in ("brand", "core_product_phrase"):
            if not title_components[label]:
                errors.append(f"Title component metadata is missing required {label}.")

    for label, component in title_components.items():
        if component and not contains_component(title, component):
            errors.append(f"Title is missing required {label}: {component}")

    if required_core_keyword:
        if not phrases_preserve_tokens_and_order(
            title_components["core_product_phrase"], required_core_keyword
        ):
            errors.append(
                "core_product_phrase must preserve every required core keyword token "
                "in its original order."
            )
        if not contains_preserved_phrase(title, required_core_keyword):
            errors.append(
                "Title must contain the complete required core keyword as one contiguous phrase: "
                + required_core_keyword
            )

    identity_components = [title_components["brand"], title_components["core_product_phrase"]]
    if all(component and contains_component(title, component) for component in identity_components):
        positions = [title.casefold().find(component.casefold()) for component in identity_components]
        if positions != sorted(positions):
            errors.append("Title identity components must place brand before core product phrase.")

    if sku_attributes:
        if sku_attribute_placement not in {"title", "item_highlights"}:
            errors.append("SKU attributes require placement 'title' or 'item_highlights'.")
        elif sku_attribute_placement == "title":
            for attribute in sku_attributes:
                if not contains_sku_attribute(title, attribute):
                    errors.append(f"Title is missing required SKU attribute: {attribute}")
        else:
            for attribute in sku_attributes:
                if not contains_sku_attribute(highlights, attribute):
                    errors.append(f"Item Highlights is missing moved SKU attribute: {attribute}")
                if contains_sku_attribute(title, attribute):
                    errors.append(f"Moved SKU attribute must not remain in Title: {attribute}")
            fit_base_title = normalize(sku_fit_base_title) or title
            title_with_sku = " ".join([
                fit_base_title.rstrip(" ,;，；"),
                *sku_attributes,
            ])
            if len(title_with_sku) <= TITLE_LIMIT:
                errors.append(
                    "SKU attributes still fit within the 75-character Title and must remain there."
                )
    elif sku_attribute_placement and sku_attribute_placement != "not_applicable":
        errors.append("SKU placement must be 'not_applicable' when no SKU attributes apply.")
    ...
    return {
        ...
        "item_highlights_phrase_count": len(highlight_phrases),
        "informative_highlight_phrase_count": len(informative_highlight_phrases),
        "errors": errors,
        "warnings": warnings,
        ...
        "component_validation": {
            **title_components,
            ...
            "sku_attribute_placement": sku_attribute_placement,
            "sku_fit_base_title": normalize(sku_fit_base_title),
...
def main() -> None:
    ...
    parser.add_argument("--core-product-phrase", default="")
    parser.add_argument("--differentiator", default="")
    parser.add_argument("--sku-attribute", action="append", default=[])
    parser.add_argument("--sku-fit-base-title", default="")
    parser.add_argument("--required-core-keyword", default="")
    parser.add_argument(
        "--sku-attribute-placement",
        choices=("title", "item_highlights", "not_applicable"),
        default="",
    )
    ...
    result = validate(
        args.title,
        args.item_highlights,
        args.marketplace,
        brand=args.brand,
        core_product_phrase=args.core_product_phrase,
        differentiator=args.differentiator,
        sku_attributes=args.sku_attribute,
        sku_attribute_placement=args.sku_attribute_placement,
        sku_fit_base_title=args.sku_fit_base_title,
        required_core_keyword=args.required_core_keyword,
    )
